...
- **工具卡片**：`╭─ ⠋ 工具名 参数摘要`（运行中 spinner）→ 结算后 `╭─ ✓/✗ 工具名`，
  结果体带左侧竖线缩进；diff 输出自动按 绿+/红- 渲染。
- **消息**：富 markdown（标题/加粗/斜体/行内代码/带语言边框的代码块/引用/有序无序列表/表格/分隔线）；
  用户输入以强调色 `❯` 前缀加粗；思考过程以斜体灰显示。
- **plan 横幅**：计划模式激活时顶部显示反色 `▌ PLAN MODE` 横幅。
- **输入面板**：圆角边框输入区 + 底部快捷键提示条；光标反色块渲染，支持多行粘贴。
- **窗口化**：按终端高度裁剪消息窗口（状态栏/输入/提示条以外的行数）。
